...
idPlayer::idPlayer() {
	...
	weapon					= NULL;
	//CPG -- declaring new speed variable and flags for classes
	newspeed				= 0.0;
	clericflag				= false;
	mageflag				= false;
	demonflag				= false;
...
// RITUAL END
	...
	if ( health > inventory.maxHealth ) {			//CPG -- return true here to have maxHealth increased and not tick down
...
void idPlayer::GiveItem( const char *itemname ) {
	...
	if ( gameLocal.mpGame.IsBuyingAllowedInTheCurrentGameMode() ) {
		...
		// CPG -- Attempt to not make inventory carry over weapons.
		if ( health <= 0 ) {
...
// RITUAL BEGIN
// squirrel: Mode-agnostic buymenus
						 //CPG edits here to set player class and give weapons based on that class
		case IMPULSE_100:	break;// CPG second test for new function //GiveItem( "weapon_shotgun" ); //test - success proper use of function
		case IMPULSE_101:   break;//AttemptToBuyItem( "weapon_machinegun" );
		case IMPULSE_102:	break;//AttemptToBuyItem( "weapon_hyperblaster" );
		case IMPULSE_103:	break;//AttemptToBuyItem( "weapon_grenadelauncher" );		break;
		case IMPULSE_104:	break;//AttemptToBuyItem( "weapon_nailgun" );				break;
		case IMPULSE_105:	break;//AttemptToBuyItem( "weapon_rocketlauncher" );		break;
		case IMPULSE_106:	break;//AttemptToBuyItem( "weapon_railgun" );				break;
		case IMPULSE_107:	break;//AttemptToBuyItem( "weapon_lightninggun" );			break;
		case IMPULSE_108:	break; // Unused
		case IMPULSE_109:	break;//AttemptToBuyItem( "weapon_napalmgun" );				break;
		...
		case IMPULSE_118:	setMage();   break;//AttemptToBuyItem( "item_armor_small" );				break;
		case IMPULSE_119:	setDemon();  break;//AttemptToBuyItem( "item_armor_large" );				break;
		case IMPULSE_120:	setCleric(); break;//AttemptToBuyItem( "ammorefill" );					break;
		case IMPULSE_121:	break; // Unused
		case IMPULSE_122:	break; // Unused
		case IMPULSE_123:	break;//AttemptToBuyItem( "ammo_regen" );					break;
		case IMPULSE_124:	break;//AttemptToBuyItem( "health_regen" );					break;
		case IMPULSE_125:	break;//AttemptToBuyItem( "damage_boost" );					break;
...
void idPlayer::AdjustSpeed( void ) {
	...
	//CPG -- Movement Speed alterations for classes

	speed *= (PowerUpModifier(PMOD_SPEED) + newspeed);
...
// squirrel: Mode-agnostic buymenus
	if ( gameLocal.isMultiplayer ) {
		if ( gameLocal.mpGame.IsBuyingAllowedInTheCurrentGameMode() ) {
			if ( gameLocal.mpGame.GetGameState()->GetMPGameState() != WARMUP ) { //Cpg changed != to ==
...
CPG - Sets the players class to a warrior, changes stats, and gives weapons based on this class
=================
*/

void idPlayer::setCleric()
{
	if(!mageflag && !demonflag && !clericflag)
	{
		if (mageflag)
		{
			common->Printf("Already a Mage\n");
		}
		else if (demonflag)
		{
			common->Printf("Already a Demon\n");
		}
		else
		{
...
CPG - Sets the players class to a mage, changes stats, and gives weapons based on this class
=================
*/

void idPlayer::setMage()
{
	if(!mageflag && !demonflag && !clericflag)
	{
		if (clericflag)
		{
			common->Printf("Already a Cleric\n");
		}
		else if (demonflag)
		{
			common->Printf("Already a Demon\n");
		...
		{
			RemoveInventoryItem( "weapon_machinegun" );
			newspeed = -.25;
			common->Printf("Hail Hydra\n");
...
CPG - Sets the players class to a Demon, changes stats, and gives weapons based on this class
=================
*/

void idPlayer::setDemon()
{
	if(!mageflag && !demonflag && !clericflag)
	{
		if (clericflag)
...
=================
idPlayer::determinespeed

CPG - Determines player speed for each class
=================
*/

float idPlayer::determinespeed()
{
	if (clericflag)
	{
		newspeed = .75;
